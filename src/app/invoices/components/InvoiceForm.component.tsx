...
export const InvoiceForm = ({ invoice, onClose }: InvoiceFormProps) => {
  ...
  useEffect(() => {
    ...
    // Prevent error in React Strict Mode double-mount
    if (!dialog.open) {
      dialog.showModal()
    }
  ...
  return (
    <dialog
      ...
      className={cn(
        ...
        // -- YOUR DRAWER STYLES --
        'fixed top-18 left-0 z-50 flex h-[calc(100vh-72px)] w-full max-w-2xl flex-col rounded-r-3xl transition-transform duration-300 lg:top-0 lg:h-screen lg:pl-25.75',
        'dark:bg-dark-bg bg-white',

        // -- THE BACKDROP --
        'backdrop:bg-black/50 backdrop:backdrop-blur-sm',

        // Optional: Native slide-in animation when the dialog opens
        'open:animate-in open:slide-in-from-left-full open:duration-300'
      )}
    >
      {/* Scrollable Content */}
      <div className="flex-1 overflow-y-auto p-6 md:p-14">
        ...
        >
          ...
          <div className="flex flex-col gap-6">
            <h4 className="text-heading-s text-primary font-bold">Bill From</h4>
            <TextInput
